...
async def main(config_dict):
    ...
    while True:
        ...
        for slot_chunk in slot_chunks: #리스트를 for loop            
            ...
            print(server_pk_list)
            await update_keywordlist(server_pk_list, data_base_info)                
            
            
                

async def update_keywordlist(server_pk_list, data_base_info):
    sql = f"""
    ...
    try:
        ...
        print(f'{server_pk_list} update error {e}')        
    finally:
        if wooriq_db:
