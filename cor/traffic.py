...
from cor.Errors import NotParsedSearchId, NotSearchedProductPrice, ServerError, WrongData
...
async def search(session: CoupangClientSession, keyword):
    ...
    async with session.get(url) as res:
        ...
        if res.status == 200:            
            ...
            raise ServerError ('serarchid url packet status not 200')
...
async def product_price_search(**kwargs):
    ...
    try:
        ...
        task = asyncio.create_task(make_coro(future))    
        await task    
        print(f"product_price update url: {update_url}")
...
async def work(slot, headers_list):
    try:
        ...
        print('클릭 완료')
        await ses.close()
    except NotParsedSearchId:
        print(f"{slot.keyword} searchid could not find")
    except ServerError as e:
        print(f"{slot.keyword}, server error {e} ")
    except FileNotFoundError as e:
